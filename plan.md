...
Introduce one prepared calculation protocol in existing owners, not a parallel framework. Separate immutable **static layout** (shapes, active masks, discretization topology, species count, model switches, dtype, sharding) from dynamic geometry/profile/Er values and mutable reuse state. A full-operator JIT probe in this review failed at `drift_kinetic.py:active_dof_mask` because it converts a traced `n_xi_for_x` with NumPy. Capturing discrete layout statically allowed a staged solve with dynamic density/temperature values; this is a useful prototype, not a complete differentiable prepared API.
