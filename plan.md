...
5. Tag `v2.4.0-rc1`. Freeze evidence tooling: no changes under `tools/benchmarks/` except bug fixes until Phase 1 needs one.
6. Fix any local checkout still on the pre-rewrite history (`git fetch && git reset --hard origin/main`).

Exit: main green at the integrated tree; ADRs 1–3 and the experiment template exist; the tag exists; the changelog holds the stack's story. Effort: one person-week, mostly review.
