...
| Size | Fresh full clone, wheel, sdist, and installed DKX-owned files each target <20 MiB. Report each measurement separately; dependencies are excluded from owned-file size, not from installation instructions. |
...
The reviewed roadmap and sparse referee correction are pushed in [PR #169](https://github.com/uwplasma/DKX/pull/169). R0 is in progress: the timeout-group fix from #168 is extended to SIGINT/SIGTERM cancellation and exited leaders, bounded-memory disk logs, full-log convergence checks, finite required moments, native SFINCS completion/Boolean encoding, and exclusion of stale copied outputs. These are execution-integrity checks, not algebraic or phase-space certificates. Local subprocess/HDF5 regressions and the validation registry pass; a live local SFINCS pilot accepts PAS and rejects finite-Er solver failure despite exit zero. Hosted gates and required review must pass before merging. PR #170 also implements atomic campaign checkpoints/completion markers, a single-writer lock, provenance-bound resume, and retention/retry of failed or cancelled attempts. Changed inputs, external equilibrium contents, code, settings or recorded runtime identity cannot silently resume an old campaign. The local validation/registry selection passes 209 tests; a live one-case DKX/SFINCS CLI pilot completes and resumes without adding a second attempt. Campaign execution completion does not certify original residual or phase-space error. The linear RHSMode 1/2/3 original-residual gate is implemented in PR #170: each RHS and cold/warm DKX output is checked against the deck tolerance, with median/raw warm timings. A four-case live pilot admits one pair; PETSc true-residual monitoring independently confirms the rejected monoenergetic/thermal reference residuals (maxima 2.6581e-8 and 2.2026e-8). The validation/registry selection passes 220 tests. Missing/nonlinear certificates stay unadmitted; explicit diagnostic mode remains available. An isolated office build of pinned SFINCS now compiles without source patches against PETSc 3.23.6, MPICH 4.3.2, MUMPS 5.8.1 and SuperLU_DIST 9.1.0 (explicit package lock and compiler log retained externally; binary SHA256 `2d4fd12971f3f981a86955c1f239e8a460ed157c6ce378665f4d28f603eb76df`). PAS passes execution/original-residual checks on MUMPS with one/two ranks (2.64e-14/3.92e-14 against 1e-10). SuperLU_DIST fails both with segmentation violations and residuals 0.068/0.053 despite reported linear convergence; its timings are excluded. The checked build reproduces the failure. An isolated four-line guard around an unconditional `MatMumpsGetInfog` call eliminates the crash and preserves the MUMPS result. A C/PETSc same-matrix known-solution replay still gives a 0.041 default SuperLU_DIST residual versus <8e-16 for MUMPS, so the numerical failure is separate. The guarded SFINCS PAS case passes at one/two ranks with explicit natural column ordering (~1.3e-14) or disabled equilibration (~9.6e-15); these configurations are smoke-qualified only, with no global-default change or production claim. The patch, build hashes, replay source and failed/successful attempts remain in external evidence; the existing reference-comparison page documents reproduction. The historical suite comparison page now labels its ratios as uncertified under current gates and documents reference-build provenance. Reference preflight now shares process-group supervision, bounded disk logs and scratch-directory isolation; timeout, early launcher-error and file-isolation regressions bring the validation/registry selection to 223 passing tests. Remaining R0 work includes nonlinear coupled-residual certification, production reference-backend qualification and the controlled CPU/GPU/PETSc pilot; native prepared-state restart remains R2 and no broad sweep is resumed yet.

R2 staging prerequisite: pitch truncation is now static, hashable operator-pytree metadata. Passing a complete, consistently rebuilt operator through a staged solve no longer traces the NumPy layout decisions. CPU operator/solver/sensitivity selection passes 100 tests; expanded staged regressions pass on CPU and an installed-wheel office GPU lane, including retracing when active pitch layout changes at unchanged rectangular shape. A 15,844-unknown full-FP GPU probe rebuilds collision data after 10% density/temperature changes, reuses one trace, satisfies the 1e-10 original-residual gate and matches cold states to relative error <4.1e-15. This is compilation reuse evidence, not a differentiable builder or prepared physical-cache/restart certificate. Remaining R2 work includes dependency-complete dynamic coefficient construction, gradients, prepared-state validity and native reuse policies.

R2 density refresh: the existing unit-density full-FP/Phi1 kernels now expose `at_uniform_density`, assembling all ordinary FP coefficients with JAX tensor operations at fixed temperature, masses, charges and speed grid. Independent rebuilds match for unequal-temperature species with independent density changes; density JVP/VJP, exact linearity and cold finite differences pass. A compiled complete-kinetic-solve derivative of a quadratic state functional matches independent cold-build finite differences at two step sizes; this does not yet certify physical flux sensitivities or temperature derivatives. The CPU collision/Phi1 regression selection passes 44 tests; all three density/Phi1 tests pass in the installed-wheel office CUDA lane, with CPU available for the adjoint diagnostic callback. The initial CUDA-only harness failure is retained as environment evidence. The expert API documents updating both density and collision coefficients and recomputing the RHS; no native restart/preconditioner-reuse claim is made.
...
| B2 | Large-aspect-ratio circular tokamak across collisionality | Banana/plateau/Pfirsch–Schlüter behavior, Shaing–Callen limits, flow/bootstrap and intrinsic ambipolarity; SFINCS plus appropriate analytic theory. |
...
| B4 | LHD/HSX/W7-X full kinetic multispecies | SFINCS, impurities/high-Z/unequal temperatures, particle/heat/current outputs; finite-Er and collision terms separated. |
...
| B10 | Linear, nonlinear and root sensitivities | Analytic/JVP/VJP/Taylor/finite-difference windows, SFINCS adjoints where supported, nonsmooth-event refusal. |
| B11 | Restart and optimization hard cases | Cross-surface 2.46% regression, wide Er reuse, `Er=15` high-pitch case; cold equivalence in observables and bounded failure recovery. |
...
5. Evaluate equilibration, mixed-precision factors plus float64 residual/refinement, adaptive rebuild and restart/recycle policy with failure envelopes. NVIDIA A4000 FP64 throughput and small-kernel launch costs constrain results. Do not infer that FP32/TF32 is accurate for ill-conditioned kinetic systems or apply GPU tuning as the CPU default.
...
| --- | --- | --- |
| **R0 Reliable evidence** | DKX; immediate | Review/adopt #168, add cancellation/descendant and atomic-output completion tests, reject partial/nonzero-exit references, repair isolated SFINCS toolchains and capture complete provenance. Close CSR/matrix-selection referee errors. One bounded idle CPU/GPU/PETSc pilot passes acceptance with no survivors; failed references remain explicit. |
| **R1 Observable correctness** | DKX + generic SOLVAX error primitives; R0 | Original residual/backward error, constrained rank/scaling, adjoint-weighted moment error, cold/warm comparison and joint-grid certificates. Reproduce the 2.46% reuse case and high-pitch current case; no promotion until published observables agree within justified errors. Marginal roots and intrinsic ambipolarity handled correctly. |
...
| **R7 Real VMEX optimization** | DKX/VMEX; R1/R2/R5, scalable R3/R4 | Fixed-equilibrium sensitivity → actual boundary objective → self-consistent bootstrap/current coupling; prescribed-Er tokamak example. Full-chain derivatives, feasible objective improvement beyond uncertainty, bounded full iteration cost, warm restart and independent final design validation. |
