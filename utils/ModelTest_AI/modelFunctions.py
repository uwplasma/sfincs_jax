...
# Initialize array
 ...
 if path:
  try:
   ...
   nun, mHats,THats,Zs,IHat,GHat,B0OverBBar,rN,nHats,dnHatdrN,dTHatdrN,Delta_SFINCS,dPhiHatdrN,iota = readH5(path,['nu_n', 'mHats','THats','Zs','IHat','GHat','B0OverBBar','rN','nHats','dnHatdrN','dTHatdrN','Delta','dPhiHatdrN','iota'],True)
   epsilont = rN / Rovera
   try:
    adiabaticTHat = float(THats[0])
   except Exception:
    adiabaticTHat = float(THats)
   try:
    adiabaticNHat = float(nHats[0])
   except Exception:
    adiabaticNHat = float(nHats)
 ...
 z_idx = zIDX if len(Zs) > zIDX else iIDX
 Zi,Ti,mi,ni,dndPsii,dTdPsii = Zs[iIDX],Ts[iIDX], ms[iIDX], ns[iIDX], dnHatdrN[iIDX], dTHatdrN[iIDX] # ion
 Zz,Tz,mz,nz,dndPsiz,dTdPsiz = Zs[z_idx],Ts[z_idx], ms[z_idx], ns[z_idx], dnHatdrN[z_idx], dTHatdrN[z_idx] # impurity
 
 # Calculate parameters
 B_poloidal = getBpol(I,r) # Poloidal magnetic field
 Itor = getItor(R0,Rbar,r,rN,B,q) # get I_t
 tauIZ,nuIz = getTauIZ(nubar,Zi,Zz,nHats[z_idx],mHats[iIDX],THats[iIDX]) # collision frequency
...
def CorrectNorm(dn,nIn):
 ...
 nz = nIn[1] if len(nIn) > 1 else nIn[0]
...
def getDensityParts(path,theta,dn):
 ...
 if path:
  ...
  z_idx = zIDX if len(nInput) > zIDX else 0
  dn=dn[0][:,z_idx,-1]
